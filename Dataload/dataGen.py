...
def randomShiftScaleRotate(image, mask,
    ...
    image = cv2.warpPerspective(image, mat, (width, height), flags=cv2.INTER_LINEAR, borderMode=borderMode,
                                borderValue=(
                                    0, 0,
                                    0,))
    # cv2.imwrite(os.path.join(pred_dir,'image%d_%d.png'%(i, st)), image)
    mask = cv2.warpPerspective(mask, mat, (width, height), flags=cv2.INTER_LINEAR, borderMode=borderMode,
                               borderValue=(
                                   0, 0,
                                   0,))
    # cv2.imwrite(os.path.join(pred_dir,'mask%d_%d.png'%(i, st)), mask)
...
class DataGenerator():
    ...
    def __getitem__(self, i):
        ...
        # mask_exp = mask.astype(np.float32)
...
if __name__ == "__main__":
    loc_path = '/media/talebi/01D7E3A66B5BE5E0/PycharmProjects/phdproject/loc_paper2/cross_valid/ACDC_nocrop5'
    ...
    acdata = load3d (train_dirpath, mask_dirpath, nb_train, **params)
    training_generator = DataGenerator(acdata, split='train')
    loader_train = data.DataLoader(training_generator, batch_size=1, shuffle=True)
    # for batch_data in loader_train:
    #     print('hello')
    for idx, batch in enumerate(loader_train):
        img, mask, fname= batch['image'], batch['mask'], batch["name"]
        print(fname, mask.shape, img.shape, mask.max(), mask.min(), img.max(), img.min())
        # print(idx)
